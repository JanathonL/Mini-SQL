...
static bplus_non_leaf *
non_leaf_new(void)
{
	bplus_non_leaf *node =(bplus_non_leaf*) calloc(1, sizeof(bplus_non_leaf));
	assert(node != NULL);
...
leaf_new(void)
{
	bplus_leaf *node =(bplus_leaf *) calloc(1, sizeof(*node));
	assert(node != NULL);
...
static int
bplus_tree_search(bplus_tree *tree, int key)
{
	int i;
...
static int
non_leaf_insert( bplus_tree *tree,  bplus_non_leaf *node,  bplus_node *sub_node, int key, int level)
{
	int i, j, split_key;
	int split = 0;
	...
	if (split) {
		...
		if (parent == NULL) {
			...
			parent->sub_ptr[0] = ( bplus_node *)node;
			parent->sub_ptr[1] = ( bplus_node *)sibling;
			parent->children = 2;
			/* update root */
			tree->root = ( bplus_node *)parent;
			tree->head[level] = ( bplus_node *)parent;
			node->parent = parent;
			...
			/* Trace upwards */
			sibling->parent = parent;
			return non_leaf_insert(tree, parent, ( bplus_node *)sibling, split_key, level + 1);
		}
	}
...
static int
leaf_insert( bplus_tree *tree,  bplus_leaf *leaf, int key, int data)
{
	int i, j, split = 0;
	 bplus_leaf *sibling;
	...
	if (split) {
		...
		if (parent == NULL) {
			...
			/* trace upwards */
			sibling->parent = parent;
			return non_leaf_insert(tree, parent, ( bplus_node *)sibling, sibling->key[0], 1);
		}
	}
...
static int
bplus_tree_insert( bplus_tree *tree, int key, int data)
{
	int i;
	 bplus_node *node = tree->root;
	 bplus_non_leaf *nln;
	...
	root->data[0] = data;
	root->entries = 1;
	tree->head[0] = ( bplus_node *)root;
	tree->root = ( bplus_node *)root;
	return 0;
}

static void
non_leaf_remove( bplus_tree *tree,  bplus_non_leaf *node, int remove, int level)
{
	int i, j, k;
	 bplus_non_leaf *sibling;
...
static int
leaf_remove( bplus_tree *tree,  bplus_leaf *leaf, int key)
{
	int i, j, k;
	 bplus_leaf *sibling;
...
static int
bplus_tree_delete( bplus_tree *tree, int key)
{
	int i;
	 bplus_node *node = tree->root;
	 bplus_non_leaf *nln;
...
void
bplus_tree_dump(bplus_tree *tree)
{
	int i, j;

	for (i = tree->level - 1; i > 0; i--) {
		bplus_non_leaf *node = ( bplus_non_leaf *)tree->head[i];
		if (node != NULL) {
			printf("LEVEL %d:\n", i);
			while (node != NULL) {
				printf("node: ");
	 ...
	 bplus_leaf *leaf = ( bplus_leaf *)tree->head[0];
	if (leaf != NULL) {
		printf("LEVEL 0:\n");
		while (leaf != NULL) {
			printf("leaf: ");
...
int
bplus_tree_get( bplus_tree *tree, int key)
{
...
int
bplus_tree_put( bplus_tree *tree, int key, int data)
{
	if (data!=-1) {
...
void Get_tree_key_and_offset(bplus_tree * tree, vector<int>& key, vector<int>& data)
{
 ...
 bplus_tree *
	bplus_tree_init(int level, int order, int entries, string table_name, string attr_name, string index_name)
{
	...
	if (tree != NULL) {
		...
		tree->attr_name = attr_name;
		tree->index_name = index_name;
		memset(tree->head, 0, MAX_LEVEL * sizeof( bplus_node *));
	}
	return tree;
}

void
bplus_tree_deinit( bplus_tree *tree)
{
	delete tree;
}

int
bplus_tree_get_range( bplus_tree *tree, int key1, int key2)
{
	int i, data = 0;
